...
async function handle_reservation_request_nyc(text_nyc, options_nyc = {}) {
  const { date_nyc, start_time_nyc, PARTY_SIZE_nyc } = parse_request_nyc(text_nyc);

  if (!date_nyc || !start_time_nyc || !PARTY_SIZE_nyc) {
    return {
      success_nyc: false,
      message_nyc:
  ...
  const { available_nyc } = await check_availability_nyc({ date_nyc, start_time_nyc, PARTY_SIZE_nyc });

  if (!available_nyc) {
    const alternatives_nyc = await suggest_alternatives_nyc({ date_nyc, start_time_nyc, PARTY_SIZE_nyc });
    if (alternatives_nyc.length > 0) {
      const suggestion_list_nyc = alternatives_nyc
        .slice(0, 5)
        .map((s_nyc) => `${s_nyc.date_nyc} at ${s_nyc.start_time_nyc}`)
        .join(', ');
      return {
        success_nyc: false,
  ...
  const { confirmation_nyc } = await make_reservation_nyc({ date_nyc, start_time_nyc, PARTY_SIZE_nyc });
  ...
  return {
    ...
    confirmation_nyc,
    details_nyc: { date_nyc, start_time_nyc, PARTY_SIZE_nyc },
    metrics_nyc,
    email_sent_nyc,
  };
}
